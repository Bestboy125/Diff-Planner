...
本轮实测 Windows 有线地址为 `192.168.5.2/24`，机载地址为 `192.168.5.5`。上行后端和
IP 白名单默认值已更新为 `192.168.5.2:8080`；主机后端的机载桥地址更新为
`192.168.5.5:50051`。token 仍为 `REQUIRED`，不允许无认证启动。
